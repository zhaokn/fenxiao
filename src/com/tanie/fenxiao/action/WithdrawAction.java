...
public class WithdrawAction extends BaseAction {
	...
	public void list() {
		...
		List withdrawList = this.withdrawService.list(hql,
	...
	public void save() {
		...
		if (this.withdraw.getMoney().doubleValue() <= 0.0D) {
			...
			if (this.withdraw.getMoney().doubleValue() > findUser.getBalance()
					.doubleValue()) {
				json.put("status", "0");
				...
				boolean result = this.withdrawService
						.saveOrUpdate(this.withdraw);
				if (result) {
					findUser.setCommission(Double.valueOf(findUser
							.getCommission().doubleValue()
							- this.withdraw.getMoney().doubleValue()));
	...
	public void info() {
		...
		try {
			if ((idStr == null) || ("".equals(idStr))) {
				...
				Withdraw findwithdraw = (Withdraw) this.withdrawService
						.findById(Withdraw.class, id);
	...
	public void update() {
		...
		try {
			if (this.withdraw == null) {
				...
				Withdraw findwithdraw = (Withdraw) this.withdrawService
						.findById(Withdraw.class, this.withdraw.getId()
								.intValue());
	...
	public void userWithdrawList() {
		...
		HttpSession session = this.request.getSession();
		User loginUser = (User) session.getAttribute("loginUser");
		String countHql = "select count(*) from Withdraw where deleted=0 and user.id="
				+ loginUser.getId();
		String hql = "from Withdraw where deleted=0 and user.id="
				+ loginUser.getId();
		hql = hql + " order by id desc";
