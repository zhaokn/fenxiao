...
public class ArticleAction extends BaseAction {
	...
	public void list() {
		...
		if (("".equals(key)) || (key == null)) {
			count = this.articleService.getTotalCount(
			...
			count = this.articleService.getTotalCount(
					"from Article where title like '%" + key
							+ "%' and deleted=0", new Object[0]);
		}

		this.page = new BjuiPage(this.pageCurrent, this.pageSize);
		this.page.setTotalCount(count);

		List list = this.articleService.list(
				"from Article where deleted=0 order by id desc",
	...
	public void info() {
		...
		try {
			...
			if ((idStr == null) || ("".equals(idStr))) {
				...
				Article findArticle = (Article) this.articleService.findById(
						Article.class, id);
				if (findArticle == null) {
	...
	public void update() {
		...
		try {
			...
			if (this.article == null) {
				...
				Article findArticle = (Article) this.articleService.findById(
						Article.class, this.article.getId().intValue());
	...
	public void indexArticleList() {
		...
		if ((idStr != null) && (!"".equals(idStr))) {
			...
			articleCate = (ArticleCate) this.articleCateService.findById(
					ArticleCate.class, Integer.parseInt(idStr));
		}
		if ((key != null) && (!"".equals(key))) {
			hql = hql + " and title like '%" + key + "%'";
		}
		hql = hql + " order by id desc";
		list = this.articleService.list(hql, pageModel.getStart(),
				pageModel.getPageSize(), new Object[0]);
		this.cfg = new Configuration();

		this.cfg.setServletContextForTemplateLoading(this.request.getSession()
				.getServletContext(), "WEB-INF/templates/index");
		Map root = new HashMap();
	...
	public void indexArticle() {
		...
		if ((idStr == null) || ("".equals(idStr))) {
			...
			if (findArticle == null) {
				...
				Config config = (Config) this.configService.findById(
						Config.class, 1);
				this.cfg = new Configuration();
