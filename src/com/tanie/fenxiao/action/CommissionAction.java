...
public class CommissionAction extends BaseAction {
	...
	public void list() {
		...
		List commissionList = this.commissionService.list(hql,
				this.page.getStart(), this.page.getPageSize(), new Object[0]);
	...
	public void save() {
		...
		try {
			if (this.commission == null) {
				...
				User findUser = this.userService.getUserByName(this.commission
						.getUser().getName());
				if (findUser == null) {
					callbackData = BjuiJson.json("300", "用户名不存在", "", "", "",
							"", "", "");
				} else {
					this.commission.setMoney(Double.valueOf(-this.commission
					...
					findUser.setBalance(Double.valueOf(findUser.getBalance()
							.doubleValue()
							+ this.commission.getMoney().doubleValue()));
					this.commission.setRemark("扣除游戏额度");
					boolean res = this.commissionService
							.saveOrUpdate(this.commission);
					if (res) {
	...
	public void info() {
		...
		try {
			if ((idStr == null) || ("".equals(idStr))) {
				...
				Commission findcommission = (Commission) this.commissionService
						.findById(Commission.class, id);
	...
	public void update() {
		...
		try {
			if (this.commission == null) {
				...
				Commission findcommission = (Commission) this.commissionService
						.findById(Commission.class, this.commission.getId()
								.intValue());
				...
				boolean result = this.commissionService
						.saveOrUpdate(this.commission);
	...
	public void userCommissionList() {
		...
		String countHql = "select count(*) from Commission where deleted=0 and user.id="
				+ loginUser.getId();
		String hql = "from Commission where deleted=0 and user.id="
				+ loginUser.getId();
		hql = hql + " order by id desc";
		...
		List ordersList = this.commissionService.list(hql,
				pageModel.getStart(), pageModel.getPageSize(), new Object[0]);
