...
public class ProductCateAction extends BaseAction {
	...
	public void list() {
		List<ProductCate> list = this.productCateService
				.list("from ProductCate where deleted=0");
		String zNodes = "";
		for (ProductCate ProductCate : list) {
			zNodes = zNodes + "<li data-id='" + ProductCate.getId()
					+ "' data-pid='" + ProductCate.getFatherId()
					+ "' data-tabid='" + ProductCate.getId() + "'>"
					+ ProductCate.getName() + "[ID:" + ProductCate.getId()
					+ "]</li>";
		}
		...
		FreemarkerUtils.freemarker(this.request, this.response,
				"productCateList.ftl", this.cfg, root);
	}
	...
	public void add() {
		...
		root.put("list", list);
		FreemarkerUtils.freemarker(this.request, this.response,
				"productCateAdd.ftl", this.cfg, root);
	}
	...
	public void getNameById() {
		...
		if ((idStr == null) || ("".equals(idStr))) {
			...
			ProductCate findProductCate = (ProductCate) this.productCateService
					.findById(ProductCate.class, id);
		...
		try {
			if ((idStr == null) || ("".equals(idStr))) {
				...
				if (findProductCate == null) {
					...
					List<ProductCate> pcs = this.productCateService
							.list("from ProductCate where deleted=0");
					String zNodes = "<li data-id='0' data-pid='0' data-tabid='0'>顶级分类</li>";
					for (ProductCate ProductCate : pcs) {
						zNodes = zNodes + "<li data-id='" + ProductCate.getId()
								+ "' data-pid='" + ProductCate.getFatherId()
								+ "' data-tabid='" + ProductCate.getId() + "'>"
								+ ProductCate.getName() + "</li>";
					...
					if (findProductCate.getFatherId() != 0) {
						ProductCate fatherProductCate = (ProductCate) this.productCateService
								.findById(ProductCate.class,
										findProductCate.getFatherId());
						if (fatherProductCate != null)
							fatherName = ((ProductCate) this.productCateService
									.findById(ProductCate.class,
											findProductCate.getFatherId()))
									.getName();
						else
					...
					FreemarkerUtils.freemarker(this.request, this.response,
							"productCateEdit.ftl", this.cfg, (Map) root);
				}
			}
		} catch (JSONException e) {
			e.printStackTrace();
	...
	public void update() {
		...
		try {
			if (this.productCate == null) {
				callbackData = BjuiJson.json("300", "参数错误", "", "", "", "", "",
						"");
			} else if (this.productCate.getFatherId() == this.productCate
					.getId().intValue()) {
				callbackData = BjuiJson.json("300", "上级分类不能选择当前修改的分类", "", "",
						"", "", "", "");
			} else {
				ProductCate findProductCate = (ProductCate) this.productCateService
						.findById(ProductCate.class, this.productCate.getId()
								.intValue());
				findProductCate.setFatherId(this.productCate.getFatherId());
				findProductCate.setName(this.productCate.getName());
				boolean result = this.productCateService
						.saveOrUpdate(findProductCate);
			...
			if ((idStr == null) || ("".equals(idStr))) {
				...
				if (findProductCate == null) {
					...
					if (sanList.size() != 0) {
						...
						boolean result = this.productCateService
								.delete(findProductCate);
						if (result)
							callbackData = BjuiJson.json("200", "删除成功",
									"ProductCateList", "", "", "true", "", "");
						else
