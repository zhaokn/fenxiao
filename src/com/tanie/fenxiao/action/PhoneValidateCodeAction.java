...
import com.tanie.fenxiao.utils.SendSms;
...
public class PhoneValidateCodeAction extends BaseAction {
	...
	public void createCode() {
		...
		if (findUser == null) {
			...
			phoneValidateCode.setCode(String.valueOf(code));
			...
			Config findConfig = (Config) this.configService.findById(
					Config.class, 1);
//			String smsContent = "您正在找回密码操作,您的短信验证码是:" + code
//					+ ",如果不是本人操作,则不用理会。【" + findConfig.getSiteName() + "】";
			try {
	...
	public void validationCode() {
		...
		String phone = this.request.getParameter("phone");
		PhoneValidateCode findPhoneValidateCode = this.phoneValidateCodeService
				.getPhoneValidateCode(phone, code);
		String callback = "";
		PrintWriter out = null;
		try {
			out = this.response.getWriter();
		...
		if (findPhoneValidateCode == null) {
			...
			if (nowTime > codeTime + 600000L) {
				...
				try {
					this.request.getRequestDispatcher("findPasswordNext.jsp")
