...
import com.tanie.fenxiao.utils.IpUtils;
import com.tanie.fenxiao.utils.Md5;
...
public class UserAction extends BaseAction {
	...
	public void list() {
		...
		if (StringUtils.isNotEmpty(key)) {
			countHql = countHql + " and (name='" + key + "' or no='" + key
					+ "' or phone='" + key + "')";
			hql = hql + " and (name='" + key + "' or no='" + key
					+ "' or phone='" + key + "')";
		}
		...
		List userList = this.userService.list(hql, this.page.getStart(),
	...
	public void register() {
		...
		String recomNo = this.request.getParameter("tuijianren");

		User recomUser = this.userService.getUserByNo(recomNo);
		...
		if (this.user == null) {
		...
		} else if (recomUser == null) {
			json.put("status", "0");
			json.put("message", "推荐人不存在");
		} else if (recomUser.getStatus().intValue() == 0) {
			json.put("status", "0");
			...
			if (StringUtils.isEmpty(recomUser.getSuperior())) {
				this.user.setSuperior(";" + recomNo + ";");
			} else if (recomUser.getSuperior().split(";").length > 3)
				this.user.setSuperior(";"
						+ recomUser.getSuperior().split(";", 3)[2] + recomNo
						+ ";");
			else {
				this.user.setSuperior(recomUser.getSuperior() + recomNo + ";");
			}
			...
			if (res) {
				User loginUser = this.userService.getUserByName(this.user
						.getName());

				loginUser.setLoginCount(Integer.valueOf(loginUser
	...
	public void update() {
		...
		try {
			if (this.user == null) {
				...
				findUser.setContacts(this.user.getContacts());
				findUser.setAddress(this.user.getAddress());
				findUser.setPost(this.user.getPost());
	...
	public void login() {
		...
		if (this.user == null) {
			...
			User loginUser = this.userService.login(this.user.getName(),
					Md5.getMD5Code(this.user.getPassword()));
			if (loginUser == null) {
				json.put("status", "0");
	...
	public void promote() {
		...
		try {
			this.request.getRequestDispatcher("promoteRegister.jsp").forward(
	...
	public void changePassword() {
		...
		String contacts = this.request.getParameter("contacts");
		String address = this.request.getParameter("address");
		String post = this.request.getParameter("post");
		HttpSession session = this.request.getSession();
		User loginUser = (User) session.getAttribute("loginUser");
		User findUser = (User) this.userService.findById(User.class, loginUser
				.getId().intValue());
		JSONObject json = new JSONObject();
		try {
			if (!StringUtils.equals(findUser.getPassword(),
					Md5.getMD5Code(oldPassword))) {
				json.put("status", "0");
				...
				findUser.setContacts(contacts);
				findUser.setAddress(address);
				findUser.setPost(Integer.valueOf(post));
	...
	public void commissionToBalance() {
		...
		if (money.doubleValue() <= 0.0D) {
			...
			if (money.doubleValue() > findUser.getCommission().doubleValue()) {
				...
				findUser.setCommission(Double.valueOf(findUser.getCommission()
						.doubleValue() - money.doubleValue()));
				this.userService.saveOrUpdate(findUser);
	...
	public void balanceToUser() {
		...
		if (toUser == null) {
			...
			if (money.doubleValue() > findUser.getBalance().doubleValue()) {
				...
				findUser.setBalance(Double.valueOf(findUser.getBalance()
						.doubleValue() - money.doubleValue()));
				toUser.setBalance(Double.valueOf(toUser.getBalance()
						.doubleValue() + money.doubleValue()));
				this.userService.saveOrUpdate(findUser);
	...
	public void levelUserList() {
		HttpSession session = this.request.getSession();
		User loginUser = (User) session.getAttribute("loginUser");
		List<User> levelUserList = this.userService.levelUserList(loginUser
				.getNo());
			...
			if (!StringUtils.isEmpty(levelNos)) {
				...
				for (int j = 1; i > 0; j++) {
					if (!StringUtils.isEmpty(leverNoArr[i])) {
						...
						if (levelUser != null) {
							if ((j == 1)
									&& (StringUtils.equals(loginUser.getNo(),
											leverNoArr[i])))
								firstLevelNum++;
							else if ((j == 2)
									&& (StringUtils.equals(loginUser.getNo(),
