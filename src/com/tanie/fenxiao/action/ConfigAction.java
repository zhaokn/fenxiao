		...
		try {
			if (loginAdmin.getJuri() != 1) {
				...
				if (findConfig == null) {
					...
					findConfig.setFirstLevel(Double.valueOf(findConfig
							.getFirstLevel().doubleValue() * 100.0D));
					findConfig.setSecondLevel(Double.valueOf(findConfig
							.getSecondLevel().doubleValue() * 100.0D));
					findConfig.setThirdLevel(Double.valueOf(findConfig
							.getThirdLevel().doubleValue() * 100.0D));
					root.put("config", findConfig);
					FreemarkerUtils.freemarker(this.request, this.response,
							"configEdit.ftl", this.cfg, root);
				}
			}
	...
	public void update() {
		...
		try {
			if (loginAdmin.getJuri() != 1) {
				...
				Config findConfig = (Config) this.configService.findById(
						Config.class, this.config.getId().intValue());
				...
				findConfig.setFirstLevel(Double.valueOf(this.config
						.getFirstLevel().doubleValue() / 100.0D));
				findConfig.setSecondLevel(Double.valueOf(this.config
						.getSecondLevel().doubleValue() / 100.0D));
				findConfig.setThirdLevel(Double.valueOf(this.config
						.getThirdLevel().doubleValue() / 100.0D));
				findConfig.setDownloadUrl(this.config.getDownloadUrl());
				findConfig.setAlipayPartner(this.config.getAlipayPartner());
				findConfig.setAlipaySellerEmail(this.config
