...
public class ProductAction extends BaseAction {
	...
	public void list() {
		...
		if (("".equals(key)) || (key == null)) {
			count = this.productService.getTotalCount(
					"select count(*) from Product where deleted=0",
					new Object[0]);
			key = "";
		} else {
			count = this.productService.getTotalCount(
					"from Product where title like '%" + key
	...
	public void update() {
		...
		try {
			...
			if (this.product == null) {
				...
				Product findProduct = (Product) this.productService.findById(
						Product.class, this.product.getId().intValue());
				...
				findProduct.setInventory(this.product.getInventory());
		...
		try {
			...
			if ((idStr == null) || ("".equals(idStr))) {
				...
				if (findProduct == null) {
					...
					try {
						boolean result = this.productService
								.delete(findProduct);
						if (result)
	...
	public void indexProductList() {
		...
		List list = this.productService.list(hql, pageModel.getStart(),
	...
	public void productDetail() {
		...
		if ((idStr == null) || ("".equals(idStr))) {
			...
			if (findproduct == null) {
				...
				try {
					this.request.getRequestDispatcher("detail.jsp").forward(
