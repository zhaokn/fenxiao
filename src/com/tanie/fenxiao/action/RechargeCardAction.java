...
import com.tanie.fenxiao.utils.Uuid;
...
public class RechargeCardAction extends BaseAction {
	...
	public void list() {
		...
		List rechargeCardList = this.rechargeCardService.list(hql,
		...
		FreemarkerUtils.freemarker(this.request, this.response,
				"rechargeCardList.ftl", this.cfg, root);
	}
	...
	public void add() {
		...
		Map root = new HashMap();
		FreemarkerUtils.freemarker(this.request, this.response,
				"rechargeCardAdd.ftl", this.cfg, root);
	}
	...
	public void save() {
		...
		try {
			...
			callbackData = BjuiJson.json("200", "成功生成" + num + "张充值卡", "", "",
					"", "true", "", "");
	...
	public void info() {
		...
		root.put("no", no);
		FreemarkerUtils.freemarker(this.request, this.response,
				"rechargeCardChongzhi.ftl", this.cfg, root);
	}
	...
	public void delete() {
		...
		try {
			...
			if ((idStr == null) || ("".equals(idStr))) {
				...
				RechargeCard findRechargeCard = (RechargeCard) this.rechargeCardService
						.findById(RechargeCard.class, id);
				if (findRechargeCard == null) {
					callbackData = BjuiJson.json("300", "充值卡不存在", "", "", "",
							"", "", "");
				} else {
					boolean result = this.rechargeCardService
							.delete(findRechargeCard);
					if (result)
	...
	public void userUseRechargeCard() {
		...
		if (findRechargeCard == null) {
			...
			findUser.setBalance(Double.valueOf(findUser.getBalance()
					.doubleValue() + findRechargeCard.getMoney()));
			...
			json.put("status", "1");
			json.put("message", "充值成功，充值金额:" + findRechargeCard.getMoney()
					+ "元");
		}
