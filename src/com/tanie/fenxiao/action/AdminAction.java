...
import com.tanie.fenxiao.utils.Connect;
import com.tanie.fenxiao.utils.DbResourcesConfiguration;
...
public class AdminAction extends BaseAction {
		...
		try {
			if (findAdmin != null) {
				...
				if (findAdmin.getStatus().intValue() == 0) {
					...
					findAdmin.setLoginCount(Integer.valueOf(findAdmin
					...
					String serverip = InetAddress.getLocalHost()
							.getHostAddress();
					String systemName = "威商科技分销系统";
					String systemVersion = "V1.0.1";
					/*
					 * String url =
					 * "http://systemapi.lxinet.com/RecordServlet?"; url = url +
	...
	public void changePwdResult() {
		...
		String callbackData = "";
		Admin findAdmin = (Admin) this.adminService.findById(Admin.class,
				loginAdmin.getId().intValue());
		try {
			if (!newPassword.equals(renewPassword)) {
				callbackData = BjuiJson.json("300", "两次输入密码不一致", "", "", "",
						"", "", "");
			} else if (!findAdmin.getPassword().equals(
	...
	public void index() {
		...
		FreemarkerUtils.freemarker(this.request, this.response, "index.ftl",
				this.cfg, root);
	}
	...
	public void add() {
		...
		this.cfg.setServletContextForTemplateLoading(this.request.getSession()
				.getServletContext(), "WEB-INF/templates/admin");
		Map root = new HashMap();
		FreemarkerUtils.freemarker(this.request, this.response, "adminAdd.ftl",
				this.cfg, root);
	}
	...
	public void database() {
		String user = DbResourcesConfiguration.getInstance().getValue(
				"jdbc.user");
		String password = DbResourcesConfiguration.getInstance().getValue(
				"jdbc.password");
		String jdbcUrl = DbResourcesConfiguration.getInstance().getValue(
				"jdbc.jdbcUrl");
		PrintWriter out = null;
		try {
			out = this.response.getWriter();
	...
	public void info() {
		...
		try {
			if (loginAdmin.getJuri() == 0) {
				...
				Admin findAdmin = (Admin) this.adminService.findById(
						Admin.class, id);
				if (findAdmin == null) {
	...
	public void update() {
		...
		try {
			...
			if (loginAdmin.getJuri() == 0) {
				...
				Admin findAdmin = (Admin) this.adminService.findById(
						Admin.class, this.admin.getId().intValue());
				if ((this.admin.getStatus().intValue() == 0)
						&& (findAdmin.getJuri() == 1)) {
					callbackData = BjuiJson.json("300", "不能禁用超级管理员帐号", "", "",
							"", "", "", "");
				} else {
					if (!"".equals(this.admin.getPassword())) {
						findAdmin.setPassword(Md5.getMD5Code(this.admin
								.getPassword()));
					}
