...
//这个就是自己要保管好的私有Key了（切记只能放在自己的后台代码里，不能放在任何可能被看到源代码的客户端程序中）
	...
	public Configure(){
		InputStream inputStream = this.getClass().getClassLoader().getResourceAsStream("wxpay.properties");
		Properties p=new Properties();
		try {
			p.load(inputStream);
			this.openId=p.getProperty("openId");
			this.key=p.getProperty("key");
			this.appID=p.getProperty("appID");
			
			this.appSecret=p.getProperty("appSecret");
			this.redirectUri=p.getProperty("redirectUri");
			this.notifyUri=p.getProperty("notifyUri");
			
			this.mchID=p.getProperty("mchID");
			this.subMchID=p.getProperty("subMchID");
	...
	public static void setRedirectUri(String redirectUri) {
		Configure.redirectUri = redirectUri;
	}
	...
	public static void setNotifyUri(String notifyUri) {
		Configure.notifyUri = notifyUri;
	}
